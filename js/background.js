...
function injectScript(tab) {
	if (!tab || !tab.id || /chrome(-extension)?:\/\/|chrome\.google\.com\/webstore/.test(tab.url)) return;
...
chrome.omnibox.onInputEntered.addListener(function(omniboxText, disposition) {
	...
	if (omniboxText === currentOmniboxQuery) query = defaultOmniboxSuggestion.content;
...
(function createBlockSiteModal() {
	var modalContent =
		"<div class='prokeys-block block-theme-plain'>\
...
chrome.tabs.onUpdated.addListener(function(tabId, info, tab) {
	chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
		if (tabs[0] && tabs[0].id === tabId) updateContextMenu();
	});
});
	...
	chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
		...
		chrome.tabs.sendMessage(tabs[0].id, { checkBlockedYourself: true }, function(response) {
			...
			contextMenuActionBlockSite =
				isBlocked === undefined ? "reload page for (un)blocking" : isBlocked ? "Unblock" : "Block";

			if (isBlocked === undefined) {
				if (recalls <= LIMIT_OF_RECALLS) setTimeout(updateContextMenu, 500, true);
				else
...
function removeCtxSnippetList(removeMainEntryFlag) {
	while (listOfSnippetCtxIDs.length > 0) chrome.contextMenus.remove(listOfSnippetCtxIDs.pop());
