...
window.Generic = function() {
	...
	this.getParentFolder = function() {
		...
		for (var i = 0, lim = index.length - 1; i < lim; i++) parent = parent.list[index[i]];
	...
	this.canNestUnder = function(newFolder) {
		if (Folder.isFolder(this)) {
			while (newFolder.name !== Folder.MAIN_SNIPPETS_NAME) {
				if (this.name === newFolder.name) return false;
	...
	this.clone = function() {
		...
		function cloneIndividualSnippetsOfFolder(folder) {
			...
			for (; i < len; i++) folder.list[i] = getClone(folder.list[i]);
		}
...
Generic.preventButtonClickOverride = function(handler) {
	return function(e) {
		if (!e.target.matches(".buttons, .buttons div")) handler.call(this, e);
	};
};

Generic.getDuplicateObjectsText = function(text, type) {
	return (
		"A " + type + " with name '" + text + "' already exists (possibly with the same letters in upper/lower case.)"
	);
};
...
Snip.makeHTMLSuitableForTextarea = function(htmlNode) {
	...
	function getProperTagPair(elm) {
		...
		if (tag === "a") returnArray[0] = "<a href='" + elm.href + "'>";
	...
	function elementSanitize(node) {
		...
		*/
		...
		if (tagName === "P" && childrenCount === 1) {
			...
			if (firstChild.tagName === "BR") return "";
			// issues#55
			else if (firstChildText.length === 0) return "";
		}
		...
		for (; i < childrenCount; i++) {
			...
			if (elm.nodeType == 1) {
				...
				if (elm.tagName === "LI") resultString += "    " + content + "\n";
...
Snip.makeHTMLValidForExternalEmbed = function(html, isListingSnippets) {
	...
	function replacerThroughArray(clsList, clsPrefix, prop) {
		for (var i = 0, len = clsList.length; i < len; i++) replacer(clsPrefix + clsList[i], prop, clsList[i]);
	}
	...
	function processPElm(pElm) {
		...
		if (!/text-align:/.test(pElm.attr("style"))) {
			...
			if (content !== "<br>" && !(pElm == $container.lastChild)) content += "<br>";
	...
	*/
	...
	if (html === DEFAULT_EMPTY_QUILL_TEXT) return "";
...
Snip.hasFormattedLossyContentWoQuillCls = function(html) {
	...
	for (var i = 0, len = regElms.length; i < len; i++) if (regElms[i].test(html)) return true;
...
Snip.getQuillClsForFormattedLossyContent = function(html) {
	...
	for (var i = 0, len = reqElms.length; i < len; i++) if (reqElms[i][0].test(html)) return reqElms[i][1];
...
Snip.stripAllTags = function(html, $refDiv) {
	...
	var children = $refDiv.childNodes,
		result = "",
		i = 0,
		len = children.length;

	if (len === 0) return $refDiv.innerText || $refDiv.textContent;

	for (; i < len; i++) result += Snip.stripAllTags("", children[i]) + "\n";
...
Snip.validate = function(arr, parentFolder, index) {
	var correctProps = ["name", "body", "timestamp"],
	...
	if (Array.isArray(arr)) {
		...
		if (propCounter !== expectedPropsLength) {
			return snippetUnderFolderString + " is missing one of the properties: " + JSON.stringify(correctProps);
		}
	}
...
window.Folder = function(name, list, timestamp, isSearchResultFolder) {
	...
	this.getLastFolderIndex = function() {
		...
		while (i < len && Folder.isFolder(this.list[i])) i++;
	...
	function adder(isSnippet) {
		...
		return function(name, body, timestamp) {
			var folderName = this.name,
				newObj;

			newObj = isSnippet ? new Snip(name, body, timestamp) : new Folder(name, body, timestamp);
	...
	this.getUniqueObject = function(name, type) {
		...
		for (var i = 0, len = index.length; i < len; i++) folder = folder.list[index[i]];
	...
	this.stripAllSnippetsTags = function($refDiv) {
		...
		this.list.forEach(function stripSnippetsOfTags(elm) {
			if (Folder.isFolder(elm)) elm.stripAllSnippetsTags($refDiv);
	...
	this.searchSnippets = function(text) {
		...
		return new Folder(
			...
			this.list
				.reduce(function(result, listElm) {
					if (Folder.isFolder(listElm)) result = result.concat(listElm.searchSnippets(text).list);

					if (listElm.matchesLazy(text)) result.push(listElm);

					return result;
				}, [])
				.sort(function(a, b) {
					return b.matchesUnique(text)
						? 1
	...
	this.toArray = function() {
		return [this.name, this.timestamp].concat(
			this.list.map(function(listElm) {
	...
	this.filterSnippetsForOmnibox = function(text, callback) {
		...
		var checkFullyLoaded = setInterval(function() {
	...
	this.getFolderSelectList = function(nameToNotShow) {
		...
		if (this.name !== Folder.MAIN_SNIPPETS_NAME) mainContainer.addClass("collapsed");

		this.forEachFolder(function(e) {
		...
		if (!hasChildFolder) mainContainer.addClass("empty");
	...
	function genericLooper(type) {
         ...
         */
		var ret = function(fn, shouldNotNest) {
			this.list.forEach(function(listElm) {
				if (!shouldNotNest && Folder.isFolder(listElm)) ret.call(listElm, fn, false);

				if (listElm.type === type) fn(listElm);
			});
		};
...
Folder.fromArray = function(arr) {
	...
	if (typeof arr[0] !== "string") arr.unshift(Folder.MAIN_SNIPPETS_NAME);

	// 2nd elm is timestamp
	if (typeof arr[1] !== "number") arr.splice(1, 0, Date.now());
	...
	folder.list = arr.map(function(listElm) {
		return Array.isArray(listElm) ? Folder.fromArray(listElm) : Snip.fromObject(listElm);
	});

	// only options page mutates list
	if (window.IN_OPTIONS_PAGE) observeList(folder.list);
...
Folder.implementChevronInFolderPath = function(notRemoveChevron) {
	...
	function computeTotalWidth() {
		var arrowCount = 0,
			totalWidth = [].slice.call($containerFolderPath.children, 0).reduce(function(sum, elm) {
	...
	if (totalWidth > width) {
		...
		if (pathPart === lastPathPart) {
			pathPart.style.width =
                $containerFolderPath.offsetWidth -
                ACTUAL_ARROW_WIDTH -
                50 + // for the chevron
                "px";
...
Folder.validate = function(arr) {
	...
	if (typeof folderName !== "string") return "Name of " + folderMsg + " is not a string.";

	folderVld = Folder.isValidName(folderName);

	if (folderVld !== "true" && Generic.getDuplicateObjectsText(folderName, Generic.FOLDER_TYPE) !== folderVld)
		return "Name of " + folderMsg + " is invalid because: " + folderVld;

	if (typeof folderTimestamp !== "number") return "Timestamp for " + folderMsg + " is not a number";
...
Folder.getDefaultSnippetData = function(){
	var name1 = "README-New_UI_Details",
		body1 =
...
window.DualTextbox = function($container, isTryItEditor) {
	...
	$nav.on(
		"click",
		function(e) {
			var node = (e.detail && e.detail.target) || e.target; // event delegation
	...
	this.userAllowsToLoseFormattingOnSwapToTextarea = function() {
		...
		if (
			detected &&
            !confirm(
            	"We detected formatted text " +
                    detected +
                    " in your expansion. You will lose it after this swap. Are you sure you wish to continue?"
            )
		)
			return false;
		else return true;
	...
	this.getShownTextForSaving = function() {
		...
		if (isCurrModePlain) return Snip.sanitizeTextareaTextForSave(this.getPlainText());
